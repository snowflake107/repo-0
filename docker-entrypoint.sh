...
# From https://github.com/mautic/docker-mautic/blob/master/apache/docker-entrypoint.sh
...
echo "#Mautic CRON" > /etc/cron.d/mautic
...
if ! [ -e index.php -a -e app/AppKernel.php ]; then
        ...
        tar zxvf /usr/src/mautic/${MAUTIC_VERSION}/www.tgz .
...
if ! [ -e app/config/local.php ]; then
        php /usr/src/mautic/${MAUTIC_VERSION}/makeconfig.php
...
if [ -n "$MAUTIC_TESTER" ]; then
  ...
  curl -LO https://raw.githubusercontent.com/mautic/mautic-tester/master/tester.php
fi
